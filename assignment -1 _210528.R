...
link <- read_html("https://www.moneyworks4me.com/best-index/nse-stocks/top-nifty50-companies-list/")
names <- html_elements(link,".company-ellipses a") %>% html_text()
cmd <- html_elements(link,"td") %>% html_text()
value = 0
for(i in 0:49)
{
...
tennis <- function(p)
{
  ...
  c = 0
  while(a!=3 && b!=3){
    a = a + sample(0:1,size = 1,prob = c(1-p,p))
    c = c + 1
    b = c - a
  }
  return(c)
